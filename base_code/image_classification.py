...
class ModelMgr():
    ...
    def get_hyperparameter(self):
        ...
        #######################
        hyper = dict()
        hyper['batch_size'] = 64  ## 배치 사이즈 설정하는 부분 입니다. 32
        hyper['epochs'] = 80  ## 에폭을 설정하는 부분 입니다. #20
        hyper['learning_rate'] = 0.01  ## 학습률을 설정하는 부분입니다. #0.01
    ...
    def get_my_model(self):
        ...
        기타 문의 : raptorjung@gmail.com (수업조교)
        '''
        model.add(keras.layers.Dense(128, input_shape=(784,), name='dense_layer', activation='elu'))
        model.add(keras.layers.Dropout(0.5))
        model.add(keras.layers.Dense(128, name='dense_layer_2', activation='elu'))
        model.add(keras.layers.Dropout(0.5))
        model.add(keras.layers.Dense(128, name='dense_layer_3', activation='elu'))
        model.add(keras.layers.Dropout(0.5))
        model.add(keras.layers.Dense(self.target_class, name='dense_layer_4', activation='softmax'))
        return model
